...
pyaop是一个通用代理模式的简单实现，用来生成一个代理对象，大绝大多数情况下，
代理对象和被代理对象除了被切入的方法外，性质保持一致，但也有特殊情况，如__eq__。
在判断两个字典是否相等时，通常判断逻辑是先判断__eq__，如果__eq__返回
NotImplemented(默认实现),则进一步对比字典中的每一个元素是否相等，但由于底层实现差异，
代理对象无法进行第二步，直接返回了第一步的结果，导致了结果上的差异。
"""
import six
